...
def get_player_url(url: str) -> str:
    if "drm" in url and "playlist.m3u8" in url:
        ...
        return f"https://patel-azure.vercel.app/?url={encrypted}"
    elif 'zip' in url:
        ...
        return f'https://patel-azure.vercel.app/?url={url}'

client = Client("patel_html_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
async def process_txt_to_html(client: Client, message: Message, theme: str):
    ...
    try:
        ...
        await msg.reply_document(document=output_path, file_name=f"{original_name}.html", caption=f"✅ Theme: `{theme}` | By PATEL")
    except Exception as e:
